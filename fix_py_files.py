...
base = r'C:\Users\Altair\Documents\Codex\2026-07-27\ban\outputs\toolsite'
...
def create_app():
    app = Flask(__name__)
    app.secret_key = 'toolsite-secret-key-change-in-production'
...
# routes.py
routes_content = r'''import hashlib, base64, urllib.parse, re, json
...
routes_path = os.path.join(base, 'app', 'routes.py')

with open(init_path, 'w', newline='\\n') as f:
    f.write(init_content.lstrip('\\n'))
print(f'Written {init_path}')

with open(routes_path, 'w', newline='\\n') as f:
    f.write(routes_content.lstrip('\\n'))
